...
class RedisSdk
{
    ...
    public function __construct($host = '127.0.0.1', $port = 6379, $auth = "1qazxsw2YD")
    {
     ...
     * @param $timeOUt
     */
    public function append($key,$value,$timeOut=0){
        $setRes = $this->redis_obj->append($key, $value);
        if ($timeOut > 0) $this->redis_obj->expire($key, $timeOut);
        return $setRes;
    }
    /**
     ...
     */
    public function keys($CheckString){
        $GetKes =   $this->redis_obj->keys($CheckString);
        if($GetKes  ===    'false') {
            return false;
        }
        return $GetKes;
